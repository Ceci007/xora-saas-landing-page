        ...
        <div className="container">
          <div className="relative flex border-2 md:flex-wrap flex-nowrap border-s3 rounded-7xl md:overflow-hidden max-md:flex-col feature-after md:g7 max-md:border-none max-md:rounded-none max-md:gap-3">
            {features.map(({ id, icon, caption, title, text, button }) => (
              <div key={id} className="relative px-5 pb-5 z-2 md:px-10 md:pb-10 flex-50 max-md:g7 max-md:border-2 max-md:border-s3 max-md:rounded-3xl max-md:flex-320">
            ...
            <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
              ...
              {details.map(({ id, icon, title }) => (
                <li key={id} className="relative px-4 pt-16 pb-14">
                  ...
                  <h3 className="relative mx-auto my-0 text-center uppercase z-2 max-w-36 base-small">{title}</h3>
                </li>
              ))}
            </ul>
          </div>
